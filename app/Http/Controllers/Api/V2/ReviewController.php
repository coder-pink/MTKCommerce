    ...
    public function submit(Request $request)
    {
        ...
        */
        ...
        foreach ($product->orderDetails as $key => $orderDetail) {
            ...
            // if($orderDetail->order != null && $orderDetail->order->user_id == auth()->user()->id && $orderDetail->delivery_status == 'confirmed' && \App\Models\Review::where('user_id', auth()->user()->id)->where('product_id', $product->id)->first() == null){
                $reviewable = true;
            }
        }
